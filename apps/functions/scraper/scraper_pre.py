...
# important for parsing the DOM (i.e. scraping search results)
from lxml import etree
# very little purpose, could be removed, but helps clean up the dom for lxml
...
import json
# Web
# the right way to handle query strings
...
from apps.functions.scraper.engines import ENGINES_BY_NAME, DdgNewsSource, Aggregator
from apps.utils.asyncify import timer_start, timer_end


async def get_df_from_search_terms(search_terms, engine_options=[(DdgNewsSource, list(), dict(max_results=200))]):
    '''
      # Timer utilities
      # times = dict()
      # def timer_start(key):
      ...
      #                     except IndexError:
      #                         match = re.match(
      #                         r"(?:\d\d? )?[JFMASOND][aepuco][nbrylgptvc][a-z]*(?: \d\d?)?(?:,? \d\d\d\d)?(?:[,/]? \d\d?:\d\d [AP]M)?|\d\d? (?:weeks|days|hours|minutes) ago", full_content)
      ...
      #                 else:
      #                     date, body = re.match(
      #                     r"^((?:\d\d? )?[JFMASOND][aepuco][nbrylgptvc][a-z]*(?: \d\d?)?(?:,? \d\d\d\d)?(?:[,/]? \d\d?:\d\d [AP]M)?|\d\d? (?:weeks|days|hours|minutes) ago) ?.? ?(.+)$", full_content).groups()
      ...
      #   def articles(self):  
      #     # Json url looks like: https://searx.elijahmendoza.eu.org/searxng/search?q=search+queries&category_general=1&pageno=1&language=en-US&time_range=None&safesearch=2&format=json
      #     res = self.request("/search?"+urlencode({"q": self.search, "format": "json", "pageno": self.pageno, **self.search_opts}, safe = "+"))
      #     data = res.json()
      #     results = data['results']
      #     # modification! Insert screaming face
      #     for result in results:
      #       result['body'] = result.get('content')
      #       result['date'] = result.get('pubdate', None)
      #       result['image'] = result.get('img_src', None)

      #     if len(data['unresponsive_engines']):
      #       print("--------\nUnresponsive engines:")
      #       print(data['unresponsive_engines'])
      #       print("--------")
      ...
      #   def to_page(self, pageno = "next"):
      #     self.pageno = (self.pageno + 1) if pageno == "next" else pageno
      #     return self.pageno
      ...
      # # Aggregator
      # # A super-news class which combines other sources with the add_source or with_source methods
      # # Example:
      # # search_term = "Elon Musk"
      # # s = (Aggregator(search_term)
      # #       .add_source(GdeltSource)
      # #       .add_source(GoogleNewsSource)
      # #       .add_source(YahooNews)
      # #       .add_source(GoogleAlerts)
    ...
    # Just add async for consistency
    async def make_searcher(search_term, engine_options):
        # Here we'll use ALL the engines because greed
        # s = (Aggregator(search_term)
        #       .add_source(DdgNewsSource, max_results=200)
        #       .add_source(SearxSearch)
        #       .add_source(GdeltSource)
        #       .add_source(GoogleNewsSource)
        #       .add_source(YahooNews)
        #       .add_source(GoogleAlerts, feeds_gsheet_csv_url="https://docs.google.com/spreadsheets/d/e/2PACX-1vR_f4MuP6pbHjQv6KRYRvvO7kmhQQ6Pgc6nttxqEaQJy8vu1yYmR1Ly8yeZkXsjQkoAjxo6UQA_4FEV/pub?gid=1148691518&single=true&output=csv")
        #       .add_source(PrivateSearx, authorization="Basic Zm9yZ2U6c2ktaW5ub3ZhdGlvbnM="))
        # Quick win
        searcher = Aggregator.from_engine_options(
            engine_options, ENGINES_BY_NAME)
        s = searcher(search_term)
        # .add_source(SearxSearch)
        # .add_source(GdeltSource)
        # .add_source(GoogleNewsSource)
        # .add_source(YahooNews)
    ...
    # Get news data from the searcher object we created
    async def get_articles(searcher, cap=1000):
        await searcher.register_sources()  # Wait for all the buggers to load

        # Then, we always need to do this first to get the first round of articles
        # Sorry for the badly-made API :)
        first_articles = await searcher.articles()
        print(f"Got {len(first_articles)} first articles")

        # Alright, now we can juice and get as many as the cap specifies
        # or at least until the engines cough up and die
        # Quality NOT guaranteed!
        await searcher.juice_all_pages(cap)

        return searcher.all_articles

    async def get_articles_from_search_term(search_term, engine_options):
        # and this is how you run an async function so colab doesn't complain
        searcher = await make_searcher(search_term, engine_options)

        # @markdown How many articles do you want?
        # TODO: get to the bottom of the context async exception thing
        max_articles = 50  # @param {type:"slider", min:0, max:5000, step:50}
        articles = await get_articles(searcher, max_articles)
        return articles

    all_articles = []
    for search_term in search_terms:
        all_articles += await get_articles_from_search_term(search_term, engine_options)

    # @title Preprocess a list of articles (generated in the previous stage)
    def preprocess_articles_to_df(articles):
        df = pd.DataFrame(articles)
        df.rename(columns={'title': 'Original_Title', 'url': 'Original_URL',
                  'date': 'Date', 'body': 'Summary'}, inplace=True)  # FORGE-friendly
        return df

    df = preprocess_articles_to_df(all_articles)

    print("Done aggregating articles")
    return df
